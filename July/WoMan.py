...
#     pc1_direction = concept_analysis["eigenvectors"][AXIS]
#     pc1_eigenvalue = concept_analysis["eigenvalues"][AXIS]
...
#     original_output = generate_with_perturbation(model, tokenizer, messages_to_test, TARGET_LAYER, pc1_direction, 0, pc1_eigenvalue)
#     print(f"Original model completion: {original_output}")
    
#     perturbation_scales = [-10.0, -5.0, -2.5, -1.5, 0.0, 1.5, 2.5, 5.0, 10.0]
    
#     print("\n--- Perturbing final token activation along PC1 ---")
#     for scale in perturbation_scales:
#         perturbed_output = generate_with_perturbation(
#             model, tokenizer, messages_to_test, TARGET_LAYER, pc1_direction, scale, pc1_eigenvalue
#         )
...
MODEL_NAME = "TinyLlama/TinyLlama-1.1B-Chat-v1.0"
# TARGET_LAYER has been removed, as we will iterate over multiple layers.
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
TORCH_DTYPE = torch.float16 if DEVICE == "cuda" else torch.float32
