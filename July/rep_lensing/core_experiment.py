        ...
        for target_word, sentences in sentences_by_target.items():
            ...
            for layer_idx in layer_indices:
                if results[target_word][layer_idx]:
                    ...
                    print(f"    Layer {layer_idx}: {results[target_word][layer_idx].shape}")
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of sentences to load per target word (default: None, loads all available)"
    )
    parser.add_argument(
        "--layers",
        nargs='+',
        type=int,
        default=[0, 15, 31],
        help="Layer indices to analyze (default: 0 15 31)"
    )
